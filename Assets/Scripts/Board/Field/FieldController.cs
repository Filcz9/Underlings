...
public class FieldController : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag == "Character" && collision.gameObject.GetComponentInParent<DragAndDrop>().isDragged)
        {
            if (dropable)
            {
            ...
            else if (collision.gameObject.GetComponentInParent<DragAndDrop>().dropSucces = true && collision.gameObject.GetComponentInParent<DragAndDrop>().dropped)
            {
                Debug.Log("click");
                ...
                collision.gameObject.GetComponent<DragAndDrop>().CheckEndField(gameObject.transform.localPosition);
                collision.gameObject.GetComponentInParent<DragAndDrop>().dropped = false;
                //collision.gameObject.GetComponent<DragAndDrop>().spriteDragEndPosition = new Vector3(0, 0, 0); //gameObject.transform.localPosition;
            }
